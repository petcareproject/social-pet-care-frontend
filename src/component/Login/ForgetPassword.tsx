...
const ForgetPassword = () => {
  ...
  const handleSubmit = async (event: { preventDefault: () => void }) => {
    ...
    try {
      ...
      const response = await http.post(USER_API.Forget_Password, formData);
      ...
      console.log("forget password response is :", response);

      toast.success("forget password succesfull");
    } catch (error) {
      setIsLoading(false);

      setError(error.message);

      console.error("password is not forget please try again", error);

      toast.error("Your password is not forget", error.response.data.message);
    }
